...
*** Variables ***
${salespersonId}    Nn4QNs7Ulk
${authCode}       819088572058440caf8320b70b8eVX84    #d8feeb06c9a041f594bbe06f2947SX53    #从弹个车客户端支付宝登录抓取login接口中获取
${shopCode}       00102063    #店铺code，一般指定002166这家测试店
${isMybankOrder}    1    #1为网商订单，0为非网商订单
${name}           陈明亮    #客户姓名
${mobile}         13575730431    #18888970448    #15869182946    #客户手机号码
${idCardNo}       130427199112120915    #411328199111057167    #331004199004190918    #332525199305240923    #客户身份证号码
${captcha}        562131    #获取到验证码后填写
${carId}          JpO8nPoWZz    #从大风车要下单的车辆抓取
${carColor}       {"color": "#FFFFFF","value": "珠光白"}    #车型颜色
${interiorColor}    {"color": "#000000","value": "黑色"}          #内饰颜色json:{value:黑色,color:'颜色编码'}
${bankCardNum}    \    #银行卡，非集采订单必填
${alipayAccount}    13575730431
${alipayUserId}    2088802650971848
${userId}         udes6b3Pkb
${iid}            1314144
${goodsIds}      ["000000856"]          #货品id列表
${alipayVersion}    1.1.1    #支付宝客户端版本，格式按照1.1.1设置即可
${productId}      101    #新车金融产品Id,二手车改字段不为空即可
...
创建90订单
    登录风车    15000001111    souche2015
    ${save}=    get_from_file    弹个车订单/新车弹个车订单/save.txt
    ${data}    Create dictionary    alipayUserId=${alipayUserId}    shopCode=${shopCode}    carId=${carId}    salespersonId=${salespersonId}    isMybankOrder=${isMybankOrder}
    ...    name=${name}    mobile=${mobile}    idCardNo=${idCardNo}    alipayAccount=${alipayAccount}    userId=${userId}    iid=${iid}     productId=${productId}
...
创建200订单，下单
    登录风车    15000001111    souche2015
    ${save}=    get_from_file    弹个车订单/新车弹个车订单/save.txt
    ${params}    Create dictionary    orderId=${save["orderId"]}    carColor=${carColor}    interiorColor=${interiorColor}    goodsIds=${goodsIds}
...
签署合同获取验证码
    ...
    ${json}=    Rest.post    /consumer/v1/agreementapi/sendCaptcha.json    ${data}    form_Auth    ${hosts["site9"]}
    Should Be True    ${json["success"]}
    log many    ${save}
    delete all sessions
...
获取taskid
    ...
    登录弹个车    ${save["alipayUserId"]}
    ${data}    create dictionary    orderId=${save["orderId"]}
    ${json}    rest.post    /consumer/v1/consumerorderapi/getConciseOrderInfo.json    ${data}    form_Auth    ${hosts["site9"]}
    Should Be True    ${json["success"]}
...
签署合同，校验验证码
    ...
    ${json}    rest.get    /consumer/v1/agreementapi/signAgreement.json    ${data}    form_Auth    ${hosts["site9"]}
    Should Be True    ${json["success"]}
    save_to_file    弹个车订单/新车弹个车订单/save.txt    ${save}
    log many    ${save}
    delete all sessions
...
关闭订单并删除相应的还款计划
    ...
    set test variable    ${order_code}    ${save["orderCode"]}
    DB.Script    finance_car_lease_v3    ${close_order}
    DB.Script    finance_car_lease_v3    ${delete_repay}
